...
systemerror[68] =
  'JavaScript boo, mostly the result of uninitialized memory access';
...
function forthkey1query() {
  if (
    !inbuf.length // key buffered - no need for event
  ) {
...
function decoded(text) {
  ...
  for (i = 0; i < text.length; i++) {
    asc = text.charAt(i);
    if (asc == '%')
      asc = String.fromCharCode(translatechar[text.charCodeAt(++i) - 65]);
    result += asc;
  }
  return result;
}
...
var controlflowwant = new Array(
  '',
  'else or then',
  'then',
  'while,until or again',
  'repeat',
  'loop or +loop',
  'next',
  'endcase',
...
function forthqsinglenumber() {
  ...
  for (
    ...
    if (digit == -2)
      exception('no input support for floating point numbers yet');
    if (digit > 9) {
      if (digit < 17) {
        tos = 0;
        break;
...
function compile() {
  for (var i = 0; i < arguments.length; jscomma(arguments[i++]));
}
...
function forthsearch1() {
  ...
  for (
    var i = wc;
    i;
    i-- // loop through headers last first
  ) {
    w = h[i];
    if (
      w // header exists
      ...
      if (
        m[context] & ds[i] // header in context dir
      ) {
        if (
          hf[i] & smudgebit // smudge bit set too ?
        ) {
          ...
          if (
            ...
            if (
              m[compliance] & ds[i] // complies with standard ?
            ) {
              tos = i; // yes, return xt
              return;
            }
...
function forthsearch() {
  ...
  if (
    !tos // go on, search voc stack if not found
  ) {
    for (var i = vocstack.length; i; ) {
      tos = 0; // assume "not found"
      i--;
      if (
        m[context] != vocstack[i] // don't search voc, if already searched
      ) {
...
screenline.push(
  ': copy (s u1 u2 -- ) swap block swap buffer c/s move update ;',
);
...
screenline.push(
  ': -->  (s -- ) blk @ 1+ block c/s !source >in off ; immediate',
);
...
screenline.push(
  ': .cell (s u -- ) 16 base exchange swap ######## space base ! ;',
);
...
function loadblockfromcookie(
  blknum,
  destaddr, // unpack block to address
) {
...
function savebuftocookie(addr, blknum) {
  ...
  for (var i = 0; i < 1024; i += cookiesectorsize)
    setcookie(
      cookiebasename + blknum + '_' + i,
      pack(addr + i, cookiesectorsize),
      null,
...
function forthblockorbuffer(
  flag, // ( u -- a )  /  flag=true:block    false:buffer
) {
  if (tos >= capacity() || tos < 0) throwerror(-35);
  if (
    blockstat[tos] >= 0 // block already mapped:
  ) {
    ...
    nextbuf %= maxbufs;
    if (
      bufblk[nextbuf] >= 0 // buffer used for another block
    ) {
...
function forthemptybuffers() {
  for (var i = 0; i < maxbufs; i++) {
    if (
      bufblk[i] >= 0 // buffer has block in ?
    ) {
...
function forthvocs() {
  for (i = vocname.length; i; type(h[vocname[--i]] + ' '));
}
...
function splitchunk(fit, newsize) {
  ...
  if (
    chunksizeleft > 0 // split chunk into two:
  ) {
...
function forthfree() {
  ...
  for (temp = 1; temp < nfreechunks; temp++) {
    if (
      freechunk[temp] == chunkaddr // merge chunks
    ) {
...
function infosall(/* No-op. */) {}
primitive('all', infosall);
describe('--', jsf);

function infostail(/* No-op. */) {}
...
describe(
  '-- a',
  'obsolete, variable containing #chars in tib',
  fig | f79 | f83 | ans,
);
...
Messenger.buffer = '';
Messenger.inputCallback = null;
Messenger.global.printed = false;
Messenger.global.finished = false;
Messenger.global.inputting = false;
Messenger.global.lines = 0;

_init();

_error = error => {
  Messenger.global.finished = true;
  return Messenger.result({ error });
};

_print = str => {
  Messenger.global.printed = true;
  return Messenger.output(str);
};

_prompt = () => {
  if (
    --Messenger.global.lines === 0 &&
    !Messenger.global.inputting &&
    !Messenger.global.finished
  ) {
    return _finish();
  }
};
...
_input = callback => {
  if (Messenger.global.finished) {
    return;
  }
  Messenger.global.inputting = true;
  Messenger.inputCallback = callback;
};
...
_finish = () => {
  ...
  inbuf = [];
  top = _stacktop(RESULT_SIZE + 1);
  if (top.length) {
    if (top.length > RESULT_SIZE) {
      top[0] = '...';
    }
    Messenger.result({ data: top.join(' ') });
  } else {
    if (Messenger.global.printed) {
      Messenger.output('\n');
    }
    Messenger.result({ data: '' });
  }
  return (Messenger.global.finished = true);
};
...
Messenger.on('evaluate', ({ code }) => {
  ...
  Messenger.global.finished = false;
  Messenger.global.inputting = false;
  Messenger.global.lines = code.split('\n').length;
  try {
    _run(code);
  } catch (_error) {
    e = _error;
...
Messenger.on('write', str => {
  Messenger.buffer += str;

  const lastCharIndex = Messenger.buffer.length - 1;
  const lastChar = Messenger.buffer[lastCharIndex];
  if (!Messenger.inputCallback || lastChar !== '\n') {
    return;
  }
  // Pass buffer but remove the newline because
  // it will be considered as part of input
  const input = Messenger.buffer.slice(0, -1);
  Messenger.buffer = '';
  (function(result) {
    var chr, i, len;
    for (i = 0, len = result.length; i < len; i++) {
      chr = result[i];
      inbuf.push(chr.charCodeAt(0));
    }
    inbuf.push(13);
    Messenger.global.inputting = false;
    Messenger.inputCallback();
    Messenger.inputCallback = null;
  })(input);
});
