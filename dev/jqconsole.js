(function() {
  ...
  Ansi = (function() {
    ...
    Ansi.prototype._remove = function() {
      ...
      for (j = 0, len1 = klasses.length; j < len1; j++) {
        ...
        if (
          klass === 'fonts' ||
          klass === 'color' ||
          klass === 'background-color'
        ) {
          results.push(
            (this.klasses = function() {
    ...
    Ansi.prototype.stylize = function(text) {
      ...
      while ((i = text.indexOf(ESCAPE_CHAR, i)) && i !== -1) {
        if ((d = text.slice(i).match(SGR_SYNTAX))) {
          ref = d.slice(1);
          ...
          text =
            this._closeSpan(text.slice(0, i)) +
            this._openSpan(text.slice(i + 1 + d[0].length));
        } else if ((d = text.slice(i).match(CSI_CATCHALL))) {
          text = text.slice(0, i) + text.slice(i + 1 + d[0].length);
        } else {
          i++;
        }
      }
  ...
  JQConsole = (function() {
    function JQConsole(
      ...
      this.$window = $(window);
      this.header = header || '';
      this.prompt_label_main =
        typeof prompt_label === 'string' ? prompt_label : DEFAULT_PROMPT_LABEL;
      ...
      this.$input_source.css({ position: 'absolute', width: 2 });
      ...
      this.$composition.css({ display: 'inline', position: 'relative' });
      this.matchings = { openings: {}, closings: {}, clss: [] };
      this.ansi = new Ansi();
      this._InitPrompt();
      this._SetupEvents();
    ...
    JQConsole.prototype._CheckKeyCode = function(key_code) {
      ...
      if (!(0 < key_code && key_code < 256) || isNaN(key_code)) {
        throw new Error(
          'Key code must be a number between 0 and 256 exclusive.',
        );
      }
      return key_code;
    };
    ...
    JQConsole.prototype.UnRegisterShortcut = function(key_code, handler) {
      ...
      removeShortcut = (function(_this) {
        return function(key) {
          if (key in _this.shortcuts) {
            if (handler) {
              return _this.shortcuts[key].splice(
                _this.shortcuts[key].indexOf(handler),
                1,
              );
            } else {
    ...
    JQConsole.prototype.ClearPromptText = function(clear_label) {
      ...
      this.$prompt_label.text(
        clear_label ? '' : this._SelectPromptLabel(false),
      );
    ...
    JQConsole.prototype.Prompt = function(
      history_enabled,
      result_callback,
      multiline_callback,
      async_multiline,
    ...
    JQConsole.prototype.AbortPrompt = function() {
      ...
      if (this.state === STATE_OUTPUT) {
        throw new Error(
          'Cannot abort prompt when not in prompt or input state.',
        );
      }
    ...
    JQConsole.prototype.RegisterMatching = function(open, close, cls) {
      var match_config;
      match_config = { opening_char: open, closing_char: close, cls: cls };
    ...
    JQConsole.prototype._InitPrompt = function() {
      ...
      this.$prompt_right.css({ position: 'relative' });
    ...
    JQConsole.prototype._HandleChar = function(event) {
      ...
      this.$prompt_left.text(
        this.$prompt_left.text() + String.fromCharCode(char_code),
      );
      this._ScrollToEnd();
      return false;
    };
        ...
        continuation = (function(_this) {
          return function(indent) {
            ...
            if (indent !== false) {
              ...
              _this.Write(
                _this.GetPromptText(true) + NEWLINE,
                CLASS_PREFIX + 'old-' + cls_suffix,
              );
              _this.ClearPromptText(true);
              if (_this.history_active) {
                if (
                  !_this.history.length ||
                  _this.history[_this.history.length - 1] !== text
                ) {
    ...
    JQConsole.prototype._GetDirectionals = function(back) {
      ...
      MoveToLimit = back
        ? $.proxy(this.MoveToStart, this)
        : $.proxy(this.MoveToEnd, this);
      MoveDirection = back
        ? $.proxy(this._MoveLeft, this)
        : $.proxy(this._MoveRight, this);
    ...
    JQConsole.prototype._VerticalMove = function(up) {
      var $prompt_opposite,
        ...
        ref,
        text;
      (ref = this._GetDirectionals(up)),
        ($prompt_which = ref.$prompt_which),
        ($prompt_opposite = ref.$prompt_opposite),
        ($prompt_relative = ref.$prompt_relative),
        (MoveToLimit = ref.MoveToLimit),
    ...
    JQConsole.prototype._HorizontalMove = function(whole_word, back) {
      var $opposite_line,
        ...
        which_end,
        word;
      (ref = this._GetDirectionals(back)),
        ($prompt_which = ref.$prompt_which),
        ($prompt_opposite = ref.$prompt_opposite),
        ($prompt_relative = ref.$prompt_relative),
        ($prompt_rel_opposite = ref.$prompt_rel_opposite),
        (which_end = ref.which_end),
        (where_append = ref.where_append);
      regexp = back ? /\w*\W*$/ : /^\w*\W*/;
      text = $prompt_which.text();
      if (text) {
        if (whole_word) {
          ...
          return $prompt_which.text(
            back ? text.slice(0, -len) : text.slice(len),
          );
        } else {
          tmp = $prompt_opposite.text();
    ...
    JQConsole.prototype._MoveTo = function(all_lines, back) {
      ...
      if (all_lines) {
        results = [];
        while (
          !($prompt_relative.is(EMPTY_SELECTOR) && $prompt_which.text() === '')
        ) {
          MoveToLimit(false);
    ...
    JQConsole.prototype._Indent = function() {
      var _;
      return this.$prompt_left.prepend(
        function() {
          var j, ref, results;
          results = [];
          for (
            _ = j = 1, ref = this.indent_width;
            1 <= ref ? j <= ref : j >= ref;
            _ = 1 <= ref ? ++j : --j
          ) {
            results.push(' ');
    ...
    JQConsole.prototype._InsertNewLine = function(indent) {
      ...
      old_prompt = this._SelectPromptLabel(
        !this.$prompt_before.is(EMPTY_SELECTOR),
      );
      $old_line = $(EMPTY_SPAN).appendTo(this.$prompt_before);
      $old_line.append($(EMPTY_SPAN).text(old_prompt));
      $old_line.append(
        $(EMPTY_SPAN)
          .addClass(CLASS_PROMPT_TEXT)
          .text(this.$prompt_left.text()),
      );
    ...
    JQConsole.prototype._ScrollPage = function(dir) {
      ...
      return this.$container.stop().animate({ scrollTop: target }, 'fast');
    };
    JQConsole.prototype._ScrollToEnd = function() {
      ...
      this.$input_container.css({ left: pos.left, top: pos.top });
      if (this.auto_focus) {
    ...
    JQConsole.prototype.ScrollWindowToPrompt = function() {
      ...
      if (
        this.isMobile &&
        (typeof orientation !== 'undefined' && orientation !== null)
      ) {
    ...
    JQConsole.prototype._WalkCharacters = function(
      text,
      char,
      opposing_char,
      current_count,
      ...
      read_char = function() {
        ...
        if (back) {
          (ref = text),
            (text =
              2 <= ref.length
                ? slice.call(ref, 0, (j = ref.length - 1))
                : ((j = 0), [])),
            (ret = ref[j++]);
        } else {
          (ref1 = text),
            (ret = ref1[0]),
            (text = 2 <= ref1.length ? slice.call(ref1, 1) : []);
        }
        if (ret) {
    ...
    JQConsole.prototype._ProcessMatch = function(config, back, before_char) {
      var $collection,
        ...
        text;
      (ref = back
        ? [config['closing_char'], config['opening_char']]
        : [config['opening_char'], config['closing_char']]),
        (char = ref[0]),
        (opposing_char = ref[1]);
      (ref1 = this._GetDirectionals(back)),
        ($prompt_which = ref1.$prompt_which),
        ($prompt_relative = ref1.$prompt_relative);
      current_count = 1;
      found = false;
      ...
      (ref2 = this._WalkCharacters(
        ...
        (index = ref2.index),
        (current_count = ref2.current_count);
      if (index > -1) {
          ...
          (function(_this) {
            return function(i, elem) {
              ...
              (ref3 = _this._WalkCharacters(
                ...
                (index = ref3.index),
                (current_count = ref3.current_count);
              if (index > -1) {
                if (!back) {
                  index--;
                }
    ...
    JQConsole.prototype._CheckMatchings = function(before_char) {
      ...
      current_char = before_char
        ? this.$prompt_left.text().slice(this.$prompt_left.text().length - 1)
        : this.$prompt_right.text()[0];
      ...
      if (before_char) {
        if (found) {
          return this._Wrap(
            this.$prompt_left,
            this.$prompt_left.html().length - 1,
            config.cls,
          );
        }
      } else {
    ...
    JQConsole.prototype._EndComposition = function() {
      ...
      this.$prompt_left.text(
        this.$prompt_left.text() + this.$composition.text(),
      );
  ...
  $.fn.jqconsole = function(
    header,
    prompt_main,
    prompt_continue,
    disable_auto_focus,
  ) {
    return new JQConsole(
      this,
      header,
      prompt_main,
      prompt_continue,
